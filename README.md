...
The directory ./Assignment_3_4/UCI HAR Dataset should now contain the following orders and files:
 ...
 7.)
The project instruction expects the following steps to tidying the raw datas:
...
Source the script 'run_analysis.R'. The script follows the project instrucions and prints the resulting
tidy data as data.frame 'avXData' to the R console. In addition it creates a directory '/data' to your
project ordner and stores the resulting tidy data from step 4 as 'tidy_XData.txt' and the tidy data from
step 5 as 'tidy_avXData.txt' in it. 
...
9.)
The resulting file 'tidy_avXData.txt' was uploaded to the github repository:
